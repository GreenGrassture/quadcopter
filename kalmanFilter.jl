...
# ╔═╡ 59f68c18-ae64-4b3a-a6f8-8745c73bc112
using DifferentialEquations.jl

# ╔═╡ 85175df3-9d8f-41a2-b079-a3e34f6a783b
using Plots

# ╔═╡ 6ec3f604-8a7c-4c81-9b7d-027de301cfcb
using LinearAlgebra

# ╔═╡ f4c13510-eff9-11eb-0ea8-e1f4eac0579f
function predictStep(A_k, B_k, x_jj, u_k, P_jj, Q_k, R_k, dt)
	# Timesteps ...i, j, k... used because k, k+1, k-1 don't work in Julia syntax
	# i is the previous timestep, j is the current one. Subscripts kj indicates that
	# the value in time k is conditioned on information up to time j, i.e. before 		# incorporating the sensor reading at time k.  A single subscript indicates no
	# dependence on previous timesteps
	
	
	# new state estimate
	x_kj = exp(dt*A_k)*x_jj + B_k*u_k
	# new estimate covariance
	P_kj = A_k*P_jj*A_k' + Q_k
	
	return x_kj, P_kj
end

# ╔═╡ d7eab6b4-8850-4ca3-8b50-776b1e70d583
function reviseStep(z_k, H_k, R_k, x_kj, P_kj)
	# Discrepancy between sensor reading that would result from current state
	# estimate, and actual measurement 
	y_k = z_k - H_k*x_kj
	
	S_k = H_k*P_kj*H_k' + R_k
	
	K_k = P_kj*H_k'*inv(S_k)
	
	x_kk = x_kj + K_k*y_k
	
end

# ╔═╡ e98d0da8-0e71-4264-877e-914091fa27ab
x_0 = [1. 0.]'

# ╔═╡ edd5d92e-a312-4190-ab8a-3d9316c2c7be
u = [0. 1.]'

# ╔═╡ df909824-97b0-4a79-a2b6-652fb4dbbf6c
P_0 = [1. 0.;
	   0. 1.]

# ╔═╡ 763108b3-8650-4465-bc9d-e5d7481c8dfb
A = [0   1;
	 0   0]

# ╔═╡ 92e54786-6278-4f4b-afc1-f7029174d5d0
B = [0. 0.;
	 0. 1.]

# ╔═╡ 53d2e71d-d1b4-481e-9913-db55644a1c54
Q = [1. 0.;
	 0. 1.]

# ╔═╡ c13ad43d-d05a-4570-b679-dcd8904b8891
R = [1. 0.;
	 0. 1.]

# ╔═╡ ada734e3-a1b8-46b5-b1ac-fcb5567deeb2
predictStep(A, B, x_0, u, P_0, Q, R, 1)

# ╔═╡ 50544a7e-9546-4eb3-beb9-5c4ff3fabd08
function truncatedExp(A, N)
	# Computes the truncated taylor series representation of exp(A) for matrix A
	trunc = I(size(A)[1])
	matProd = I(size(A)[1])
	divisor = 1.
	for n in 1:N
		matProd = matProd*A
		divisor = divisor*n
		trunc = trunc + matProd/divisor
	end
	return trunc
end

# ╔═╡ 9d5365e2-c4a2-46fd-a2cb-ea0a94592096
M = [1 1 -3;
	 0 2 0;
	 1 1 4]*1

# ╔═╡ 12f9f2fb-6223-40eb-82c3-e7aef0c91945
truncatedExp(M, 100)

# ╔═╡ 49be6bdf-6e5a-4706-9849-97093f28b15a
exp(M)

# ╔═╡ 2145864a-a8eb-4881-879a-c07d8f3717b1
function plotEigSequence!(mat, N, plt)
	allEigs = [eigvals(truncatedExp(mat, n)) for n in 1:N]
	for n in 1:N
		eigs = allEigs[n]
		xs = [real(eig) for eig in eigs]
		ys = [imag(eig) for eig in eigs]
		scatter!(plt, xs,ys)
	end
end

# ╔═╡ 10a3356d-2775-4e1a-9867-b77c02ce3b9e
plt = plot()

# ╔═╡ 54e278ef-095d-4234-8554-142e3368bfc5
plotEigSequence!(M, 3, plt)
